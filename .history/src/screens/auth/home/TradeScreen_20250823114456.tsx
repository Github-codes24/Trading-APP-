...
const TradeScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleConnectionStatus = (status: { connected: boolean; error: string | null }) => {
      console.log('Connection status changed:', status);
      if (status.connected) {
        setConnectionError(false);
        setConnectionErrorMessage('');
        setIsLoading(true); // Still loading until we get data
      } else if (status.error) {
        setConnectionError(true);
        setConnectionErrorMessage(status.error);
        setIsLoading(false);
      }
    };
    tradingApiService.subscribeToConnectionStatus(handleConnectionStatus);

    // Subscribe to real data from WS
    const handleLiveData = (data: TradingInstrument[]) => {
      console.log('Received trading data:', data?.length);
    ...
    const dataTimeout = setTimeout(() => {
      if (tradingData.length === 0 && !connectionError) {
        console.warn('No data received within 15 seconds, showing error');
        setConnectionError(true);
        setConnectionErrorMessage('No trading data received. The server may be unavailable.');
        setIsLoading(false);
      }
    }, 15000);
      ...
      {connectionError ? (
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorMessage}>
            {connectionErrorMessage || 'Unable to connect to trading server. Please check your internet connection and try again.'}
          </Text>
          <TouchableOpacity 
