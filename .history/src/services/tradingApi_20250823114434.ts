...
class TradingApiService {
  ...
  initializeConnection() {
    ...
    try {
      ...
      this.socket.onopen = () => {
        ...
        this.emitLocal('connectionStatus', { connected: true, error: null });
      ...
      this.socket.onclose = (event) => {
        ...
        if (event.code !== 1000 && event.code !== 1001) {
          ...
          this.emitLocal('connectionStatus', { connected: false, error: null });
        }
      };
    } catch (error) {
      console.error('❗ Failed to initialize trading socket:', error);
      this.handleConnectionError(`Failed to initialize connection: ${error}`);
      this.tryReconnect();
    }
  }
  ...
  private handleConnectionError(errorMessage: string) {
    ...
    this.emitLocal('connectionStatus', { connected: false, error: errorMessage });
  }
  ...
  subscribeToConnectionStatus(callback: (status: { connected: boolean; error: string | null }) => void) {
    console.log('👂 Subscribing to connection status events');
    this.onLocal('connectionStatus', callback);
  }
